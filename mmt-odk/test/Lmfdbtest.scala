...
object Lmfdbtest {
  def main (args: Array[String]) {
    ...
    val c = controller.get(Path.parse("http://www.lmfdb.org/db/elliptic_curves?curves?11a1"))
    val d = controller.get(Path.parse("http://www.lmfdb.org/db/elliptic_curves?curves?35a2"))
    println(c)
    println(d)
  }
}
