...
class MacroUsingPresenter extends ObjectPresenter {
  ...
  private def doObj(obj: Obj)(implicit context: Context): String = {
    ...
    // mark as inferred if there is no source ref; TODO also marks all subexpressions of an inferred expression
    //if (parser.SourceRef.get(obj).isEmpty)
    //  "{\\mmt@inferred{" + objS + "}}"
    objS
  }
