...
import info.kwarc.mmt.stex.xhtml.{XHTML, XHTMLMath, XHTMLNode, XHTMLOMDoc, XHTMLText, XMathML}

import scala.xml.{Elem, Node}

trait DocumentExtension extends STeXExtension {
  def documentRules : List[PartialFunction[XHTMLNode,Unit]]
}
...
object DocumentExtension extends STeXExtension {
  ...
  def doDocument(uri : String) = {
    ...
    implicit val xhtmlrules = exts.flatMap(_.xhtmlRules)
    val filecontent = getDocument(uri)
    val docrules = exts.collect {
      case e : DocumentExtension =>
        e.documentRules
    }.flatten
    def doE(e : XHTMLNode) : Unit = docrules.foreach(r => r.unapply(e))
    filecontent.iterate(doE)
    filecontent
  }

  override def doHeader(head: XHTMLNode,body: XHTMLNode): Unit = {
    XHTML.applyString("<html><head>" + MMTSystem.getResourceAsString("mmt-web/stex/overlay.txt") + "</head></html>")(Nil).head.children.head.children.foreach(head.add)
  ...
  def getDocument(uri : String) : XHTMLNode = {
    implicit val rules = server.extensions.flatMap(_.xhtmlRules)
    uri match {
      case "http://mathhub.info/fomid/demo.xhtml" =>
        XHTML.applyString(MMTSystem.getResourceAsString("mmt-web/stex/demo/test.xhtml")).head
            ...
            case Some(a) =>
              val cnt = a.properties.collectFirst {
                case ("description",d) if (a.root / "META-INF" / d).exists() =>
                  XHTML.applyString(File.read(a.root / "META-INF" / d)).head
                case ("description",d) if (a.root / "meta-inf" / d).exists() =>
                  XHTML.applyString(File.read(a.root / "meta-inf" / d)).head
                case ("teaser",t) =>
                  XHTML.applyString("<div>"+t+"</div>").head
              }
            ...
            case Some(a) =>
              ...
              if (fn.exists() && fn.isDirectory) {
                if ((fn / "index.xhtml").exists()) {
                  val ret = XHTML.applyString(File.read(fn / "index.xhtml")).head
                  server.doHeader(ret)
                  ret
                } else {
                  val (doc, bd) = server.emptydoc
                ...
                val ret = XHTML.applyString(File.read(fn)).head
                server.doHeader(ret)
                ret
              }
              else {
  ...
  def sidebar(elem : XHTMLNode, content: List[Node],atend : Boolean = false) = {
    ...
    )(Nil).head.children.map(makesafe)
  ...
  def overlay(elem : XHTMLNode, urlshort : String,urllong : String) = {
    def criterion(n : XHTMLNode) = {
      n.getClass == classOf[XHTMLNode] || n.getClass == classOf[XHTMLText] || n.getClass == classOf[XHTMLMath] || n.getClass == classOf[XMathML]
    } && !n.attributes.contains(("stex","arg"))
    ...
    val t = elem.top
    val id = t.generateId
    val overlay = XHTML.apply(<span style="position:relative"><iframe src=" " class="stexoverlay" id={id} onLoad='this.style.height=(this.contentWindow.document.body.offsetHeight+5) + "px";'>{XHTML.empty}</iframe></span>)(Nil).head
