...
case class ExecFile(file: File, name: Option[String]) extends ExecAction with ActionWithErrorRecovery {
  def apply(errorCont: Option[ErrorHandler]): Unit = {controller.runMSLFile(file, name, true, errorCont)}
...
trait ExecActionHandling{self: Controller =>
  ...
  def runMSLFile(f: File, nameOpt: Option[String], showLog: Boolean, errorCont: Option[ErrorHandler]): Unit = {
