...
abstract class Obj extends Content with ontology.BaseType with ShortURIPrinter with HashEquality[Obj] {
   ...
   /** the bound variables of this object in any order */
   lazy val boundVars : List[LocalName] = boundVars_.distinct
   /** all variables (free or bound) of this object in any order */
   lazy val allVars : List[LocalName] = (freeVars_ ::: boundVars_).distinct
   ...
   /** helper function for boundVars that computes the bound variables without eliminating repetitions */
   private[objects] def boundVars_ : List[LocalName]
...
case class OMID(path: ContentPath) extends Term {
   ...
   private[objects] def boundVars_ = Nil
   private[objects] def paths_ = List(path)
   def subobjects = Nil
...
case class OMBINDC(binder : Term, context : Context, scopes: List[Term]) extends Term  {
   ...
   private[objects] lazy val boundVars_ = binder.boundVars_ ::: context.boundVars_ ::: context.variables.map(_.name).toList ::: scopes.flatMap(_.boundVars_)
...
case class OMA(fun : Term, args : List[Term]) extends Term {
   ...
   private[objects] lazy val boundVars_ = fun.boundVars_ ::: args.flatMap(_.boundVars_)
...
case class OMATTR(arg : Term, key : OMID, value : Term) extends Term {
   ...
   private[objects] def boundVars_ = arg.boundVars_ ::: value.boundVars
...
case class OMSemiFormal(tokens: List[SemiFormalObject]) extends Term with SemiFormalObjectList {
   ...
   private[objects] def boundVars_ = tokens.flatMap(_.boundVars)
...
case class OML(name: LocalName, tp: Option[Term], df: Option[Term], nt: Option[TextNotation] = None, featureOpt : Option[String] = None) extends Term with NamedElement {
   ...
   /**
    ...
    private[objects] def boundVars_ = vd.boundVars
