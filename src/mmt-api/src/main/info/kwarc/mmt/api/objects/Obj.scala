...
abstract class Obj extends Content with ontology.BaseType with ShortURIPrinter with HashEquality[Obj] {
   ...
   /** returns the subobject at a given position and its context
     * @param pos Position of the subobject
     * @param cmlXPath if true, the position is an XPath in the content MathML, not a position as generated by .subobjects
     */
   def subobject(pos: Position, cmlXPath: Boolean = false) : (Context, Obj) =
     pos.indices.foldLeft((Context(),this)) {
         case ((con,obj), i) =>
            // some objects have a name as the first subobjects in the content MathML, so the XPath positions are 1 too big
            val cmlOffset = if (!cmlXPath) 0 else this match {
              case _:VarDecl | _:Sub => 1
              case _ => 0
            }
            obj.subobjects.lift(i-cmlOffset) match {
