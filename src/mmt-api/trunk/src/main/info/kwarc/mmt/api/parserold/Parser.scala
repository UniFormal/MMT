...
import objects._
import frontend._
import parser.{SourceRef,SourceRegion,SourcePosition,TermParser,ParsingUnit,ParsingError}
...
class OldNotationParser(grammar : Grammar, controller: Controller) extends TermParser {
  ...
  private def makeOperators(scope : List[Content]) : List[Operator] = {
    scope collect {
      case c : Constant => new Operator(c.path, TextNotation(Nil, 0, false, c.path))//c.not)
