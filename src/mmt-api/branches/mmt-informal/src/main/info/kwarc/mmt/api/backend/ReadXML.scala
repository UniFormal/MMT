   ...
   def readInDocument(nsMap: NamespaceMap, docOpt: Option[Document], node : Node)(implicit cont: StructuralElement => Unit) {
      ...
      node match {
         ...
         case <sref/> => 
             val t = xml.attr(node, "target")
             log("mref to " + t + " found")
	         val r = SRef(doc.path, Path.parseS(t,nsMap), false)
	         add(r)
         case <group/> => 
             val xrefs = node.child map {c =>
               val t = xml.attr(node, "target")
               c.label  match {
                 case "dref" => DRef(doc.path, Path.parseD(t,nsMap), false)
                 case "mref" => MRef(doc.path, Path.parseM(t,nsMap), false)
                 case "sref" => SRef(doc.path, Path.parseS(t,nsMap), false)
               }
             }
   ...
   def readInModule(home: MPath, nsMap: NamespaceMap, moduleOpt: Option[StructuralElement], node: Node)(implicit cont: StructuralElement => Unit) {
      ...
      symbol match {
         ...
         case <flexiformal>{decls @ _*}</flexiformal> =>
            ...
            val fd = FlexiformalDeclaration.parseDeclaration(symbol, name , home, nsMap.base)
            add(fd)
         case scala.xml.Comment(_) =>
