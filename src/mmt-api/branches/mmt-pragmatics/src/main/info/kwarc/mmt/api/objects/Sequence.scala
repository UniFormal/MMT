...
object Seq {
	def apply(items : List[SeqItem]) = SeqItemList(items)
	def unapplySeq(tm : SeqItemList) : List[SeqItem] = tm.items
}
...
object Rep {
	def apply(fn : Term, n : Term): SeqItem = SeqSubst(fn, "", SeqUpTo(n))
    def unapply(t : SeqItem) : Option[(Term,Term)] = t match {
		case SeqSubst(fn,"",SeqUpTo(n)) => Some((fn, n))
		case _ => None
	}
}
...
object RepInt {
    ...
    def unapply(t : SeqItem) : Option[(Term,Int)] = t match {
		case SeqSubst(t,"",SeqUpTo(OMI(n))) => Some((t,n.toInt))
		case _ => None
	}
