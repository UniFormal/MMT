...
class ServerState(archiveRoot: File, debug: Boolean)extends Logger {
...
*/
  ...
  // mutable List
  private val lobbies = mutable.ListBuffer[LobbyState]()
 ...
 */
  ...
  def initLobbyState_a(): LobbyState = {
    val ls: LobbyState = null
    if(lobbies.isEmpty) {
      val nls = new LobbyState(archiveRoot, debug)
      lobbies += nls
      nls
    }else{
      ls
...
override def log(s: => String, subgroup: Option[String] = None): Unit = super.log(s, subgroup)
  ...
  override protected def report: Report = ??? //todo
}
