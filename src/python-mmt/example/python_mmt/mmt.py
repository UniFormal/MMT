...
JavaMember.__doc__  = "doc strings not accessible in Scala"
JavaObject.__doc__  = "doc strings not accessible in Scala"
def eagerGetAttr(obj,a):
   """eagerly builds the table of all attributes, then does the lookup;
   standard behavior of __getattr__ in Py4J is to lazily return a JavaMember even if none exists, i.e., hasattr would always return True"""
   obj._populate_fields()
   aG = obj._methods.get(a)
   if aG == None:
      # returning None is not sufficient, need to raise an error
      raise AttributeError("method " + a + " does not exist")
   else:
      return aG
JavaObject.__getattr__ = lambda self,x: eagerGetAttr(self,x)
# in test
...
class StructuralElement():
  """wraps around an MMT structural element and interprets it as a Python object in terms of getDeclarations and getComponents"""
  def __init__(self, se):
    self.se = se
  def __repr__(self):
    return "<MMT element " + self.se.path() + ">"
  def __str__(self):
    return self.se.toString()
  def __dir__(self):
    return self.se.domain()
  def __getattr__(self,a):
    aG = self.se.getO(a)
    if aG.isEmpty():
      raise AttributeError("unknown name: " + str(a))
    else:
      return StructuralElement(aG.get())