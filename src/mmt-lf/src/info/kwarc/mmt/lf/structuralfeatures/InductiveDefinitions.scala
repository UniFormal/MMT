...
class InductiveDefinitions extends StructuralFeature("inductive_definition") with ParametricTheoryLike {
  ...
  def elaborate(parent: Module, dd: DerivedDeclaration) = {
    ...
    val indTplPath = dd.getDeclarations.head match {case c: Constant if (c.name.toString() == "tpl") => c.tp.get match {case OMS(p) => p}}
    ...
    val inductiveTypeDecl = controller.library.getO(indDerDeclPath).getOrElse(throw LocalError("No corresponding inductively-defined type found at "+indDerDeclPath.toString()))
    val inductiveType = inductiveTypeDecl match {
        case dd : DerivedDeclaration if (dd.feature == "inductive") => dd
        case _ => throw LocalError("Expected corresponding inductively-defined type at "+indDerDeclPath.toString()+" but no derived declaration of deature inductive found.")
    }
    ...
    val indTplDef = decls.find(_.name == indTplName).get.df.get
    val Tp = PiOrEmpty(context, Arrow(indTplDef, indTpl.toTerm))
