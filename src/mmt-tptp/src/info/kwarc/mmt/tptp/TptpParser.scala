...
class TptpParser {
  ...
  def parseString: Any = {
    try {
      ...
      case _: Exception => log("Parsing term failed")
    }
    try {
      return parser.plain_term(outputManager)
    } catch {
      case _: Exception => log("Parsing plain_term failed")
    }
    try {
      return parser.defined_term(outputManager)
    } catch {
      case _: Exception => log("Parsing defined_term failed")
    }
    try {
      return parser.system_term(outputManager)
    } catch {
      case _: Exception => log("Parsing system_term failed")
    }
    try {
      return parser.fof_formula(outputManager)
    } catch {
      case _: Exception => log("Parsing fof_formula failed")
    }
    try {
      return parser.unitary_formula(outputManager)
    } catch {
      case _: Exception => log("Parsing unitary_formula failed")
    }
    try {
      return parser.quantified_formula(outputManager)
    } catch {
      case _: Exception => log("Parsing quantified_formula failed")
    }
    try {
      return parser.literal(outputManager)
    } catch {
      case _: Exception => log("Parsing literal failed")
    }
    try {
      return parser.tptp_literals(outputManager)
    } catch {
      case _: Exception => log("Parsing tptp_literals failed")
    }
    try {
      return parser.tptp_literal(outputManager)
    } catch {
      case _: Exception => log("Parsing tptp_literal failed")
    }
    try {
      return parser.general_term(outputManager)
    } catch {
      case _: Exception => log("Parsing general_term failed")
    }
    try {
      return parser.general_data(outputManager)
    } catch {
      case _: Exception => log("Parsing general_data failed")
    }
    try {
      return parser.general_list(outputManager)
    } catch {
      case _: Exception => log("Parsing general_list failed")
    }
    try {
      return parser.info_item(outputManager)
    } catch {
      case _: Exception => log("Parsing info_item failed")
    }
    try {
      return parser.formula_role(outputManager)
    } catch {
      case _: Exception => log("Parsing formula_role failed")
    }
    try {
      return parser.annotations(outputManager)
    } catch {
      case _: Exception => log("Parsing annotations failed")
    }
    try {
      return parser.tptp_input(outputManager)
    } catch {
      case _: Exception => log("Parsing tptp_input failed")
    }
    try {
      return parser.topLevelItem(outputManager)
    } catch {
      case _: Exception => log("Parsing topLevelItem failed")
    }
    null
  }
}
