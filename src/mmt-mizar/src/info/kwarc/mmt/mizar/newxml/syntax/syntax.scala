...
case class Item(kind: Kind, pos:Positions, _subitems:List[Subitem])
trait Subitem
...
case class Pragma(_notionName: Option[Notion_Name], _canceled:Option[Canceled]) extends Subitem
...
case class AggregateFunctor_Pattern(extPatDef: ExtPatDef) extends Patterns
case class ForgetfulFunctor_Pattern(extPatDef: ExtPatDef) extends Patterns
...
case class InfixFunctor_Pattern(orgPatDef: OrgPatDef) extends Patterns
case class CircumfixFunctor_Pattern(orgPat: OrgPatAttrs, _right_Circumflex_Symbol: Right_Circumflex_Symbol, _loci:List[Locus], _locis:List[Loci]) extends Patterns
case class SelectorFunctor_Pattern(extPatDef:ExtPatDef) extends Patterns
...
case class Existential_Quantifier_Formula(srt:Sort, pos:Position, _vars:Variable_Segments, _expression:Formula) extends Formula
case class Relation_Formula(objectAttrs: OrgnlExtObjAttrs, leftargscount:Int, _args:Arguments) extends Formula
case class Universal_Quantifier_Formula(srt:Sort, pos:Position, _vars:Variable_Segments, _restrict:Option[Restriction], _expression:Formula) extends Formula
...
case class Canceled(MmlId:MMLId, amount:Amount, kind:Kind, position:Position)
...
case class Field_Segment(pos:Position, _selectors:Selectors, _standardType:Standard_Type)
...
case class Properties(prop:Option[PropertyAt], _cond:List[Properties])
case class Redefine(occ:Occurs)
case class Type_Specification(_types:List[Type])
case class Definiens(pos:Position, kind:Kind, shape:String, _label:Label, _expr:Expression)
...
case class Notion_Name(pos:Position, insc:Inscription)
