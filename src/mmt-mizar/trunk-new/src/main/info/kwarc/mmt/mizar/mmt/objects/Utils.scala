...
object Mizar {
	val MizarTh = DPath(mmt.baseURI / "set_theories" / "mizar" / "mizar-curry.omdoc") ? "Mizar-Curry"
	val HiddenTh = DPath(mmt.baseURI / "set_theories" / "mizar" / "HIDDEN.omdoc") ? "HIDDEN"
...
object MMTUtils {
	...
	def getFilePath(aid : String) : DPath =  {
		if (aid == TranslationController.currentAid) 
			...
			DPath(mmt.baseURI / "set_theories" / "mizar" / (aid + ".omdoc"))
	}
