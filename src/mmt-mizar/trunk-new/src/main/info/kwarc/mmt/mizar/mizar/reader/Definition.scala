...
object DefinitionParser {
	...
	def parseDefinition(n : Node) : MizAny = {
		 ...
		 if (nCons.length == 1) {
		   ...
		   val selectors = xmlsel.map(x => new MizSelector(ParsingController.resolveDef(x.aid, "U", x.nr),x.aid, x.nr)).toList
		   new MizStructDef(ParsingController.resolveDef(xmlstr.aid, "L", xmlstr.nr), xmlstr.aid, xmlstr.nr, xmlstr.argTypes, selectors)		   
		 } else {
			 new MizErr()
		 }
