	...
	def buildDefinitions(d : XMLDefinition, art : MizArticle) :  Unit =  {
			val df = definiens.find(x => (d.nr == x.absconstrnr && d.kind == x.constrkind && d.aid == x.constraid))  match {
				...
				case rd : XMLRedefinition => {
					new XMLIsDefiniens(d.aid, d.nr : Int, d.aid, d.kind, d.nr,Nil, Some(new MizFunc(rd.aid + "_" + rd.kind + "_" + rd.nr ,rd.constraid, d.kind, rd.constrabsnr, rd.argTypes.zipWithIndex.map(x => new MizLocusVar(x._2 + 1)))))//TODO check
				}
	...
	*/
	...
	def resolveDef(aid : String, kind : String, absnr : Int) : Option[String] = {
		aid+kind+absnr match {
			case "HIDDENM1" => Some("set")	
			case "HIDDENR1" => Some("==")
			case "HIDDENR2" => Some("in")
			case _ => dictionary.symbols.find(x => ((x.aid == Some(aid)) && (x.kind == kind) && (x.absnr == absnr))) match {
				case Some(x) => {
					Some(x.name)
