 ...
 */
class MMTError(val mainFile: File, es: ErrorSource, el: Level.Level, sf: String, sl: Int, sc: Int, ec: Int, msg: String)
...
class ErrorListForwarder(errorSource: MMTErrorSource, controller: Controller, mainFile: File) extends ErrorHandler {
   ...
   protected def addError(e: Error) : Unit = e match {
      case s: SourceError =>
         ...
         val error = try {
           val (line,column) = if (pos.line == -1 || pos.column == -1) {
             ...
             (pos.line,pos.column)
           new MMTError(mainFile, errorSource, s.level, file, line, column, column + reg.length, s.mainMessage)
         } catch {
           case t : Exception =>
      ...
      case e: Invalid =>
         ...
         val reg = ref.region
         val error = new MMTError(mainFile, errorSource, e.level, mainFile.toString,
      ...
      case e: Error =>
         ...
         val error = new MMTError(mainFile,
             errorSource, e.level, mainFile.toString, 0, 0, 1, "error with unknown location: " + e.getMessage
         )
