...
object Query {
  def command(arg: String): String = {
    ...
    def add_content(kind: String)(content: Importer.Content, entry: Entry): Importer.Content = {
    ...
    val content1 = classes.foldLeft(Importer.Content.empty)(add_content(Importer.Main.Kind.CLASS))
    val content2 = types.foldLeft(content1)(add_content(Importer.Main.Kind.TYPE))
    val content3 = consts.foldLeft(content2)(add_content(Importer.Main.Kind.CONST))
