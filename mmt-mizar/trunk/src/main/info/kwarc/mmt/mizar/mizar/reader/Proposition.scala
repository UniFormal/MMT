...
object PropositionParser {
	...
	def parseFormula(n : scala.xml.Node) : MizFormula = n.label match {
		...
		case "Pred" => {
			val p = new MizPred((n \ "@aid").text, (n \ "@kind").text, (n \ "@absnr").text.toInt, n.child.map(TypeParser.parseTerm).toList)
			p.setName(ParsingController.resolveDef((n \ "@pid").text, p.kind, p.absnr))
			p
		}	
		case "PrivPred" => {
