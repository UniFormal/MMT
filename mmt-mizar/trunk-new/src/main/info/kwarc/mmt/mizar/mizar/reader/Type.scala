...
object TypeParser {
	...
	def parseTerm(n : scala.xml.Node) : MizTerm = {
		n.label match {
			...
			case "Func" =>  {
				...
				val kind = (n \ "@kind").text
				val absnr = (n \ "@absnr").text.toInt
				val name = ParsingController.resolveDef(aid, kind, absnr) match {
					case Some(s) => s
					case None => aid + "_" + kind + "_" + absnr
				}
				new MizFunc(name, aid, kind, absnr, n.child.map(parseTerm).toList) //TODO
			}
			case "PrivFunc" => 
	...
	def parseAdjective(n : Node) : MizAdjective = {
		...
		new MizAdjective(name, value, terms)
	}
