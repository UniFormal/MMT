...
/**
	...
	def parseNotationPattern(n : Node) {
		...
		val name = ParsingController.dictionary.getNameByFormatnr(kind, formatnr) match { 
			case None => 
			println("this should not happen:" + aid + kind + formatnr + constrAid)
			aid + "_" + kind + "_notation_" +  formatnr //TODO change to symbolnr or remove
			case Some(s) => s//TODO integrate with resolveDef from controller
		}
		println(name)
		//ParsingController.dictionary.addAbsnr(kind, formatnr, nr);

		val constrName = ParsingController.resolveDef(constrAid, kind, absconstrnr) match {
			case Some(s) => s
			case None => //ParsingController.dictionary.symbols.map(s => println(s.name + " " + s.aid + " " + s.kind + " " + s.absnr + " " + s.symbolnr + " " + s.formatnr))
			constrAid + "_" + kind + "_" + absconstrnr
		}
		println(constrName)
		val not = new MizNotation(name, aid, kind, constrAid, constrName, antonymic)
