 ...
 */
class ErrorListForwarder(errorSource: DefaultErrorSource, controller: Controller, file: File) extends ErrorHandler {
   def addError(e: Error) = e match {
      case s: SourceError =>
      ...
      case e: Invalid =>
         ...
         val cause: metadata.HasMetaData = e match {
            ...
            case e: InvalidUnit =>
               ...
               steps.find(_.isInstanceOf[WFJudgement]) match {
                  case Some(j: WFJudgement) =>
                     mainMessage += ": " + controller.presenter.asString(j.wfo)
                     j.wfo
                  case _ =>
                     controller.localLookup.get(e.unit.component.parent)
               }
         }
         val ref = SourceRef.get(cause) getOrElse {
            mainMessage = "unlocatable error: " + mainMessage
            SourceRef(utils.FileURI(file), SourceRegion(SourcePosition(0,0,0), SourcePosition(0,0,0)))
         }
         val reg = ref.region
         val error = new DefaultErrorSource.DefaultError(errorSource, ErrorSource.ERROR, file.toString,
      ...
      case e: Error =>
         ...
         val error = new DefaultErrorSource.DefaultError(
             errorSource, ErrorSource.ERROR, file.toString, 0, 0, 1, "unknown error in file: " + e.getMessage
         )
         errorSource.addError(error)
   }
}
