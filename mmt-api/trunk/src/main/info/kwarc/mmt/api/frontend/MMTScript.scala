...
/** .mbt files should contain a single object that extends MMTScript */
abstract class MMTScript extends Extension {
  ...
  def loadExtension(uri: String, args: List[String] = Nil): Unit = controller.handle(AddExtension(uri, args))

  def config: MMTConfig = controller.config

  //Utility

  def runImporters(aid: String, btm: BuildTargetModifier = Build): Unit = {
    config.getArchive(aid).formats.flatMap(config.getImporters).distinct foreach { imp =>
      build(List(aid), imp, btm)
    }
  }

  def runExporters(aid: String, btm: BuildTargetModifier = Build): Unit = {
    config.getArchive(aid).formats.flatMap(config.getImporters).distinct foreach { imp =>
      build(List(aid), imp, btm)
    }
  }
  ...
  def parse(f: String, autoload: Boolean = true): Unit = {
    val file = File(f)
