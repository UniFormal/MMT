...
trait Tactic extends Rule {
    ...
    * }}}
    */
   protected def onApply(cont: => Alternative) = {
...
trait ForwardInvertible extends InvertibleTactic {
   def apply(prover: P, context: Context): Option[ApplicableTactic]
   def apply(prover: P, g: Goal): Option[ApplicableTactic] = apply(prover, g.context)
}
