...
abstract class BuildTarget extends Extension {
   ...
   protected def delete(f: File, extraExt : List[String] = Nil) {
       log("deleting " + f)
       f.delete
       extraExt foreach {e => f.setExtension(e).delete}
   }
}
...
trait GenericTraversingBuildTarget extends BuildTarget {
   ...
   /** the file extensions of other files that are generated by buildFile or buildDir
    *  
    *  defaults to Nil
    *  
    *  these will additionally be deleted when calling Update or Delete
    */
   def additionalOutExt: List[String] = Nil
       ...
       a.traverse(inDim, in, _ => true) {case Current(inFile, inPath) =>
          a.timestamps(key).modified(inPath) match {
             case Deleted =>
                ...
                delete(outFile, additionalOutExt)
             case Added =>
                buildAux(inPath)(a)
             case Modified =>
                val outFile = outPath(a.root, inPath)
   ...
   def clean (a: Archive, args: List[String], in: List[String] = Nil) {
       a.traverse(outDim, in, _ => true) {case Current(inFile, _) =>
          delete(inFile, additionalOutExt)
       }
   }
