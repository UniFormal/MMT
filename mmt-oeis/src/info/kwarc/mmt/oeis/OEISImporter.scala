...
case class OEISSourceError(msg : String, sref : Option[SourceRef], severity : Option[Level.Level]) extends OEISError(msg) {
  ...
  override def toNode = <error type={this.getClass().toString} shortMsg={this.shortMsg} level={severity.getOrElse(Level.Error).toString} sref={srefS}> {this.extraMessage} </error>
}
