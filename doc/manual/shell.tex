...
\begin{center}
\code{java -cp scala/*;mmt-api.jar info.kwarc.mmt.api.RunNull}
\end{center}
Further command line parameters are passed to the shell and executed as a command (see below). In particular,
\begin{center}
  \code{java -cp mmt-api.jar info.kwarc.mmt.api.RunNull file F}
\end{center}
can be used to execute a startup/configuration file \code{F}. After invocation, the shell can be controlled via STDIN/STDOUT.

The \code{-cp} parameter defines the Java classpath. Besides the main file \code{mmt-api.jar}, it must contain the jar file(s) of the Scala library \cite{scala}, which is here assumed to reside in a folder \code{scala}. Note that the classpath is a list of entries separated by \code{;} on Windows and \code{:} on Unix.
...
\begin{center}
\code{java -Xmx1024m -cp scala/*;mmt-api.jar info.kwarc.mmt.api.RunNull}
\end{center}
