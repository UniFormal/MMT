The {\mmt} language and its semantics have been described in depth in \cite{rabe:mmt:09}. Here we will focus on describing the XML syntax to someone who already has a general understanding of {\mmt}.
...
MMT URI         & \mmturi  & $\bnfas$ & $D\bnfalt M\bnfalt S$    \\
Document URI    & $D$      & $\bnfas$ & $\op{URI}$, no query, no fragment  \\
Module URI      & $M$      & $\bnfas$ & $D?\mmtqname \bnfalt ?(/\mmtname)^*$  \\
Symbol URI      & $S$      & $\bnfas$ & $M?\mmtqname \bnfalt ??(/\mmtname)^*$ \\
  & $\op{URI}$, $pchar$ & & see RFC 3986~\cite{BerFieMas:05} \\
\hline
\end{tabular}
\end{center}

We will use $Q$ and $R$ to range over qualified names. In a URI $D[?Q[?R]]$, $D$, $Q$, and $R$ are called the document, module, and symbol part.
A URI is called \defemph{absolute} if the document part is an absolute URI and if --- if present --- neither $Q$ nor $R$ are of the form $(/\mmtname)^*$. Otherwise, it is called \defemph{relative} or an MMT URI \defemph{reference}.
...
relative to \ldots & 1. \ldots document URI   & 2. \ldots module URI  & 3. \ldots symbol URI \\ \hline
...
\begin{enumerate}
	\item $u$ starts with a URI reference $d$: If $D'$ is the result of resolving $d$ against the document part of $D$ of $U$ according to RFC 3986, then the resulting MMT URI is $D'$, $D'?q$, or $D'?q?r$ depending on the form of $u$. \\
	...
	\item $u$ is of the form $?q_{ref}$ or $?q_{ref}?r$ where $q_{ref}$ is of the form $(/\mmtname)^*$: If $U$ is of the form $D?Q$ or $D?Q?R$, then $u$ is interpreted relative to $D?Q$ yielding $D?Qq_{ref}$ or $D?Qq_{ref}?r$, respectively. It is an error if $U$ is a document URI.
	In particular, the special cases where $q_{ref}$ is empty yield $D?Q$ and $D?Q?r$, respectively.
	\item $u$ is of the form $??r_{ref}$ where $r_{ref}$ is of the form $(/\mmtname)^*$: If $U=D?Q?R$ is a symbol URI, then the resolution yields $D?Q?R/r$. It is an error if $U$ is a module or document URI.
	In particular, the special case where $r_{ref}$ is empty yields $D?Q?R$.
\end{enumerate}
\medskip
