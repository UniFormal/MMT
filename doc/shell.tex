...
\item \code{archive ID D F}: This builds the dimensions \code{D} in the archive with id \code{ID}, optionally restricted to the folder/file \code{F}. Legal values for \code{D} are \code{compile}, \code{content} (which will produce \texttt{content} and \texttt{narration}), \code{mws}, and \code{relational}.
