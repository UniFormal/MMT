...
object Test {
	...
	def classToLF(c : OWLClassExpression) : Term = { //p?  
		c match {
		    ...
		    /*case c : OWLComplementOf =>
		        val name = new LocalPath("_")
		        val args = c.getOperand
		        ApplySpine(OWLOMS("OWLDL", "ComplementOf"), args : OWLClassExpression)
		    */   
	...
	def individualToLF (i : OWLIndividual) : Term = { //p?
		i match {
	...
	def objectToLF (o: OWLPropertyAssertionObject) : Term = {null} //p?
	
	val OWL1 = new DPath(new jomdoc.utils.xml.URI("http://cds.omdoc.org/logics/description/owl/owl.omdoc")) //p*

	def OWLOMS (t: String, s: String) = {  //t-module s-name" //*
		OMS(OWL1 ? t ? s) //owl1- base   bu symbolu yaratiyor
	}

/* 	def OWLOMA(f: Term, args: List[Term]) = {
		OMA(f,args) 
	}
*/

	def IRItoLocalPath(i: IRI) = new LocalPath(i.toString) //p*

	def CurrOMS(i: IRI) = OMS(currentThy ? IRItoLocalPath(i)) //p??
...
*/
		    ...
		    val thy = new Theory(dpath, LocalPath(List("_")), Some(OWL1 ? "OWLFull")) // buradan aldim
			controller.add(thy)
			currentThy = thy.path
			...
			axioms.foreach(ax => ax match {
				...
				case ax : OWLDeclarationAxiom =>
				 	...
				 	if(entity.isBuiltIn())
						   ...
						   val name = IRItoLocalPath(entity.getIRI) // woman  //p*
						   val tp  = OWLOMS("OWLBase", "class")     // type , module and name //p*
						   val con = new Constant(currentThy, name, Some(tp), None, null) //p*
						     // theory name: ex, class name:woman,type,none definition icin, null ignore et //p* 
						   println("new: " + con.toString) //p*
					} 
				...
				case ax : OWLObjectPropertyRangeAxiom =>
			      println("_" + " : " + "(" + "Range" + ax.getProperty + ax.getRange + ")" + ".")
			   ...
			   // case ax : OWLObjectPropertyAxiom =>
			    case ax : OWLTransitiveObjectPropertyAxiom =>	
				   println("_" + " : " + "(" + "transitiveProperty" + ax.getProperty +  ")" + ".")
		...
		/*		case ax : OWLEquivalentObjectPropertiesAxiom =>	
				 //?? println("_" + " : " + "(" + "equivalentProperty" + ? ")" + ".")
				  
				   val name = new LocalPath("_") //p*
				   val args = ax.getProperties.map(propertyToLF) // new?
				   val tp = ApplySpine(OWLOMS("OWLLite", "equivalentProperty"), args : _*) //?
				   val con = new Constant(currentThy, name, Some(tp), None, null)
				   controller.add(con)
