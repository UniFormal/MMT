...
function showComp(comp){
	populateDialog('latin-dialog', "<img src='"+ gContext + "/../../classpath/ajax-loader.gif'></img>", 'Definition Lookup', false);
...
function continuation(data, comp){
	var selector = 'latin-dialog';
	populateDialog(selector,data, comp);	
}
...
for (x in loadedModules) {
	if (loadedModules[x][0] == "latin") {
		console.log("Successfully registered latin");
		loadedModules[x][2] = latin;
		break;
	}
}
