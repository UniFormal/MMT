...
/**
 * loadLib - function that loads a javascript libo the xhtml document.
 * Additionally it also populates the list of loaded libs for further reference.
 *
 * @param context : the context of the web application, can be '', which makes
 *   scripts being resolved relatively to the HTML file that includes them
 * @param moduleName : name of the module to be included
 * @param hostAddress: address of the host that serves the service
 * @throws an alert if the moduleName is not recognized or invalid
 */
function loadLib(context, moduleName, hostAddress){
    if (hostAddress === undefined) {
        hostAddress = "";
    }
    if (context == null) 
        context = gContext;
    //testing for correctness and validity of user-entered moduleName
    var loaded = false;
    var file = hostAddress + context + '/' + moduleName + '.js';
    var moduleCheck = function(){
        for (x in loadedLibs) {
            if (file == loadedLibs[x][1]) {
                loaded = true;
                return false;
            }
        }
    ...
    if (moduleCheck) {
        include(file);
        var myMod = [moduleName, file]; //mod is defined in each service file and keeps a reference to the instantiated service
        loadedLibs.push(myMod);
    } // else inform the user that something went wrong
    else {
        if (loaded == true) 
            console.warn("Service: " + moduleName + " is already loaded.");
        else 
            console.warn("Oops! Something went wrong for: " + moduleName);
    }
}

/**
 * loadModule - function that loads a javascript module to the xhtml document.
 * Additionally it also populates the list of loaded modules for further reference.
 *
 * @param context : the context of the web application, can be '', which makes
 *   scripts being resolved relatively to the HTML file that includes them
 * @param moduleName : name of the module to be included
 ...
 * @throws an alert if the moduleName is not recognized or invalid
 */
function loadModule(context, moduleName, hostAddress){
    if (hostAddress === undefined) {
        hostAddress = "";
    }
    gHost = hostAddress;
    //testing for correctness and validity of user-entered moduleName
    var loaded = false;
    var file = context + '/' + moduleName + '.js';
    var moduleCheck = function(){
        for (x in loadedModules) {
            if (file == loadedModules[x][1]) {
                loaded = true;
                return false;
            }
        }
    ...
    // if moduleCheck test is passed then load the module
    if (moduleCheck) {
        gContext = context;
        //make sure the path to the jqcontextmenu arrow is ok
        jquerycontextmenu.arrowpath = context + "/../../../classpath/arrow.gif";
        gContext = gContext.replace('services', 'lib');
        include(file);
        var myMod = [moduleName, file, null, hostAddress]; //mod is defined in each service file and keeps a reference to the instantiated service
        loadedModules.push(myMod);
    } // else inform the user that something went wrong
    else {
        if (loaded == true) 
            console.warn("Service: " + moduleName + " is already loaded.");
...
$(document).ready(function(){
	$('body').append('<div id="_jobad_dialog" title="Basic dialog" />');
