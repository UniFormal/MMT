package jomdoc.patterns
import jomdoc._
import jomdoc.libraries._
import jomdoc.objects._
import jomdoc.symbols._
import jomdoc.objects.Context._
import jomdoc.objects.Substitution._
...
object PRep {
	def apply(fn: Term, n: Int): Term = OMA(jomdoc.utils.mmt.repetition,List(fn,OMI(n)))
    def unapply(t: Term) : Option[(Term,Int)] = t match {
		case OMA(jomdoc.utils.mmt.repetition,List(fn,OMI(n))) => Some((fn,n.toInt))
		case _ => None
	}
}
...
object Pattern {
  ...
  def splitTerm(tm: Term, ctx: Context = Context()):(Context,Term) = { //TODO: If we have a fixed joker context, then we dont need splitting.
	  tm match {
	 	  case OMBIND(bin,con,bdy) if bin == jomdoc.utils.mmt.jokerbinder => splitTerm(bdy,ctx ++ con)
	 	  case _ => (ctx,tm)
	  }
  }
  ...
  def expandRepetitionInd(tm: Term): Term = {
	  tm match {
	 	  ...
	 	  val expargs = 
	 	 	  args flatMap {
	 	    	 case OMA(fn,List(f,OMI(n))) if (fn == jomdoc.utils.mmt.repetition) => 
	 	    	 //Currently no nested repetitions, thus no recursion
	 	           List.tabulate[Term](n.toInt)(i => OMA(jomdoc.utils.mmt.index,List(OMI(i + 1),f)))
  ...
  def expandRepetition(tm: Term): Term = {
	  tm match {
	 	  ...
	 	  val expargs = 
	 	 	  args flatMap {
	 	    	 case OMA(fn,List(f,OMI(n))) if (fn == jomdoc.utils.mmt.repetition) => //Currently no nested repetitions, thus no recursion
  ...
  def substituteList(tm: Term, vr:String, tl: List[Term]): Term = {
	   tm match {
	 	  case OMA(jomdoc.utils.mmt.index,List(OMI(i),fn)) => 
	 	    OMA(jomdoc.utils.mmt.index,List(OMI(i),fn ^ Substitution(Sub(vr,tl(i.toInt - 1)))))
  ...
  def removeIndex(tm:Term): Term = {
	   tm match {
	  	   case OMA(jomdoc.utils.mmt.index,List(OMI(i),fun)) => fun
