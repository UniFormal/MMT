...
class Get {
   ...
   private val path = Path.parse(new jomdoc.utils.xml.URI(doc), mod, sym, basepath)
   /** the main snippet retrieving the requested object and applying the requested action */
   def get : Node = {
      try {Controller.doGet(doc, mod, sym, act)}
      catch {case e @ jomdoc.backend.NotFound(p) => scala.xml.Text(e.getMessage)}
   }
