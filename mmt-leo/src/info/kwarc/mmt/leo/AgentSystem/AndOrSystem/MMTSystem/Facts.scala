 ...
 */
class Facts(blackboard:LFBlackboard,shapeDepth: Int) extends Debugger {

  //val report = prover.report
  def logPrefix = "Fact Database"

  lazy val controller = blackboard.controller
  ...
  */
  def termsOfTypeAtGoal(goal: LFProofTree, queryVars: Context, query: Term): List[(Substitution, Term)] = {
  ...
  */
  def solutionsOfGoal(goal: LFProofTree): List[Term] = {
  ...
  *  If not all queryVars occur in query, then sub will be partial.
  */
  def termsOfTypeBelowGoal(goal: LFProofTree, queryVars: Context, query: Term): List[(Substitution, Term, Option[LFProofTree])] = {
    var res: List[(Substitution, Term, Option[LFProofTree])] = Nil
    foreachFact(queryVars, query) {case f =>
       if (goal isBelow f.goal) {
  ...
  def has(g: LFProofTree, tp: Term): Option[Term] = {
